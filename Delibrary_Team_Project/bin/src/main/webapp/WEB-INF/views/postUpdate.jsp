		        ...
		        <div style="border: 1px solid #eee; padding: 10px; margin: 10px 10px 10px 0px;">
			        <form action="postUpdate.do" method="post" enctype="multipart/form-data">
						...
						<div id="p_title">
							...
							<input type="text" name="p_title" value="${p.p_title }">
							<input type="hidden" name="p_id" value="${p.p_id }">
							<input type="hidden" name="cust_no" value="${p.cust_no }">
							<br>
							<%-- <p style="font-size: 15px; color: #aaa;">게시일 ${p.p_regdate} | 작성자 ${p.p_writer} | 조회 ${p.p_hit }</p> --%>
						</div>
						<div id="p_content">
							<textarea rows="15" cols="90" name="p_content">${p.p_content }</textarea>
						</div>
